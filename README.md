...
1. Parses the draft into sections
2. Retrieves the relevant product facts and writing/compliance rules for each section
3. Uses an LLM to compare the draft against that reference material
4. Returns a structured, validated list of issues (factual errors, unsupported claims, writing violations) with severity, reasoning, and source citations
5. Persists the review in SQLite so follow-up questions ("why was issue 3 flagged?") can be answered from the saved data
6. Supports natural conversation — review requests, follow-up questions, and general chat are routed to the right handler automatically
